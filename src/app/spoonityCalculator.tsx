...
export default function SpoonityCalculator() {
  ...
  React.useEffect(() => {
    const storedToken = localStorage.getItem("spoonity_calculator_token");
    const storedExpiry = localStorage.getItem("spoonity_calculator_expiry");
    ...
    if (storedToken && storedExpiry) {
      ...
      if (isValid) {
        ...
        // Initialize SMS country based on stored country
        const storedCountry = localStorage.getItem("spoonity_user_country");
        if (storedCountry === "Other") {
          setSmsEnabled(false);
          setSmsMessages("");
          setSmsCountry("USA");
        } else {
          setSmsCountry(storedCountry || "USA");
        }
      } else {
        // Clear expired data
        localStorage.removeItem("spoonity_calculator_token");
        localStorage.removeItem("spoonity_calculator_expiry");
        setTokenError("Your session has expired. Please log in again.");
      }
    }
  }, []);

  // Token validation and generation functions
  const generateToken = () => {
    ...
    localStorage.setItem("spoonity_calculator_token", token);
    localStorage.setItem("spoonity_calculator_expiry", expiry.toString());
  ...
  const validateToken = () => {
    ...
    if (!isValid) {
      // Clear all localStorage data when token expires
      localStorage.removeItem("spoonity_calculator_token");
      localStorage.removeItem("spoonity_calculator_expiry");
      localStorage.removeItem("spoonity_user_firstName");
      localStorage.removeItem("spoonity_user_lastName");
      localStorage.removeItem("spoonity_user_email");
      localStorage.removeItem("spoonity_user_phone");
      localStorage.removeItem("spoonity_user_company");
      localStorage.removeItem("spoonity_user_role");
      localStorage.removeItem("spoonity_user_country");
      localStorage.removeItem("spoonity_user_otherCountry");
      localStorage.removeItem("spoonity_user_businessType");
  ...
  React.useEffect(() => {
    ...
    }, 1000 * 60 * 10); // Check every 10 seconds
  ...
  React.useEffect(() => {
    if (!isTokenValid || !tokenExpiry) return;
  ...
  const [firstName, setFirstName] = React.useState(
    () => localStorage.getItem("spoonity_user_firstName") || ""
  );
  const [lastName, setLastName] = React.useState(
    () => localStorage.getItem("spoonity_user_lastName") || ""
  );
  const [email, setEmail] = React.useState(
    () => localStorage.getItem("spoonity_user_email") || ""
  );
  const [phone, setPhone] = React.useState(
    () => localStorage.getItem("spoonity_user_phone") || ""
  );
  const [company, setCompany] = React.useState(
    () => localStorage.getItem("spoonity_user_company") || ""
  );
  const [role, setRole] = React.useState(
    () => localStorage.getItem("spoonity_user_role") || ""
  );
  const [country, setCountry] = React.useState(
    () => localStorage.getItem("spoonity_user_country") || "USA"
  );
  const [otherCountry, setOtherCountry] = React.useState(
    () => localStorage.getItem("spoonity_user_otherCountry") || ""
  );
  const [businessType, setBusinessType] = React.useState(
    () => localStorage.getItem("spoonity_user_businessType") || "corporate"
  );
  ...
  const handleLogin = (e: React.FormEvent): void => {
    ...
    localStorage.setItem("spoonity_user_firstName", firstName);
    localStorage.setItem("spoonity_user_lastName", lastName);
    localStorage.setItem("spoonity_user_email", email);
    localStorage.setItem("spoonity_user_phone", phone);
    localStorage.setItem("spoonity_user_company", company);
    localStorage.setItem("spoonity_user_role", role);
    localStorage.setItem("spoonity_user_country", country);
    localStorage.setItem("spoonity_user_otherCountry", otherCountry);
    localStorage.setItem("spoonity_user_businessType", businessType);
  ...
  const getQuoteDetails = () => {
    ...
    if (whatsappEnabled) {
      addOns.push(
        `WhatsApp Platform (${whatsappCountry}): ${formatCurrency(
    ...
    if (giftCard) {
      addOns.push(
        `Gift Card Program: ${formatCurrency(feeBreakdown.giftCard)}/month`
      );
    }
  ...
  const generatePDF = () => {
    ...
    try {
      ...
      if (typeof quoteData === "object") {
        ...
        doc.text(
          `Business Type: ${
            businessType === "corporate" ? "Corporately Owned" : "Franchised"
          }`,
          20,
          77
        );
        ...
        doc.setFont("helvetica", "normal");
        doc.text(
          `Monthly Recurring Fees: ${formatCurrency(monthlyFees)}`,
          20,
          yPos + 59
        );
        ...
        if (businessType === "franchise") {
          doc.text(
            `  Corporate Monthly Fee: ${formatCurrency(
              feeBreakdown.corporate
            )}`,
            30,
            yPos + 66
          );
          doc.text(
            `  Franchisee Fee per Store: ${formatCurrency(
              feeBreakdown.franchiseePerStore
            )}`,
            30,
            yPos + 73
          );
        ...
        doc.text(
          `Monthly Fee per Store: ${formatCurrency(perStore)}`,
          20,
          yPos + 66
        );
        doc.text(
          `One-Time Setup Fees: ${formatCurrency(setupFees)}`,
          20,
          yPos + 73
        );
        doc.text(
          `First Year Total Cost: ${formatCurrency(
            monthlyFees * 12 + setupFees
          )}`,
          20,
          yPos + 80
        );
        ...
        const splitDisclaimer = doc.splitTextToSize(
          "This is an estimated quote based on the information provided. A Spoonity representative will contact you to provide a final quote and answer any questions.",
          170
        );
        //@ts-ignore
        ...
        y += 10;
        doc.text(
          `First Year Total: ${formatCurrency(monthlyFees * 12 + setupFees)}`,
          20,
          y
        );
        y += 20;
  ...
  const calculateConnectionFees = (storeCount: number): number => {
    ...
    if (storeCount <= 10) {
      ...
      connectionFees =
        10 * 150 + 15 * 135 + 15 * 121.5 + (storeCount - 40) * 109.35;
    }
  ...
  function calculateFees() {
    ...
    if (transactionVolume <= 5000) {
      ...
      transactionFees =
        5000 * 0.005 + 45000 * 0.003 + (transactionVolume - 50000) * 0.002;
    }
                ...
                <div className="flex flex-col sm:flex-row gap-4 justify-center">
                  <button
                    onClick={() => {
                      ...
                      alert(
                        "Please copy the text below for your records:\n\n" +
                          quoteText
                      );
                    }}
                    ...
                    <div className="space-y-4">
                        ...
                        <div className="space-y-2 text-right font-medium">
                          ...
                          <p>
                            {formatCurrency(
                              stores <= 10
                                ? stores * 150
                                : stores <= 25
                                ? 10 * 150 + (stores - 10) * 135
                                : stores <= 40
                                ? 10 * 150 + 15 * 135 + (stores - 25) * 121.5
                                : 10 * 150 +
                                  15 * 135 +
                                  15 * 121.5 +
                                  (stores - 40) * 109.35
                          ...
                          <p>
                            {formatCurrency(
                              0.25 *
                                (stores * transactions) *
                                (0.25 * (stores * transactions) <= 5000
                                  ? 0.005
                                  : 0.25 * (stores * transactions) <= 50000
                                  ? 0.003
                                  : 0.002)
                            )}
                          </p>
                            ...
                            <p>
                              {formatCurrency(
                                marketing <= 100000
                                  ...
                                  : marketing <= 1000000
                                  ? 100000 * 0.0006 +
                                    (marketing - 100000) * 0.0004
                                  : 100000 * 0.0006 +
                                    900000 * 0.0004 +
                                    (marketing - 1000000) * 0.0002
                              )}
                            </p>
                          )}
                          {plan !== "loyalty" && pushNotifications && (
                        ...
                        <div className="border-t pt-4 mt-4">
                          ...
                          <div className="grid grid-cols-2 gap-4 text-sm">
                            <div className="space-y-2 text-gray-500">
                              ...
                              {whatsappEnabled && (
                                <p>WhatsApp Platform ({whatsappCountry})</p>
                              )}
                            ...
                            <div className="space-y-2 text-right font-medium">
                              {giftCard && (
                                <p>{formatCurrency(feeBreakdown.giftCard)}</p>
                              )}
                              {smsMessages && parseInt(smsMessages) > 0 && (
                            ...
                            <div className="mt-3 text-xs bg-gray-50 p-3 rounded-md">
                              ...
                              <div className="grid grid-cols-2 gap-1">
                                ...
                                <span className="pl-2">
                                  • OTP Messages ({whatsappOtp.toLocaleString()}{" "}
                                  × $
                                  {whatsappRates[whatsappCountry].otp.toFixed(
                                    2
                                  )}
                                  ):
                                </span>
                                ...
                                <span className="text-right font-medium pt-1 border-t mt-1">
                                  {formatCurrency(
                                    feeBreakdown.whatsapp.messages
                                  )}
                                </span>
                              </div>
                            </div>
                      ...
                      <div className="border-t border-b py-4 my-2">
                        <div className="grid grid-cols-2 gap-4 text-sm">
                          ...
                          <div className="text-right font-medium">
                            {formatCurrency(perStore)}
                          </div>

                          {businessType === "franchise" && (
                            <>
                              <div className="col-span-2 mt-4 mb-2">
                                <h3 className="font-semibold text-base text-gray-800 border-b pb-1">
                                  Suggested Cost Breakdown for Franchise Model
                                </h3>
                              </div>
                              ...
                              <div className="col-span-2 bg-purple-50 rounded-lg p-4 mb-3 border border-purple-200">
                                ...
                                <div className="grid grid-cols-2 gap-2 text-sm mb-3">
                                  ...
                                  <div className="text-gray-700 font-semibold pt-2 border-t mt-1">
                                    Total Corporate Monthly Cost:
                                  </div>
                                  <div className="text-right font-semibold pt-2 border-t mt-1">
                                    {formatCurrency(feeBreakdown.corporate)}
                                  </div>
                                ...
                                <div className="text-xs text-gray-600">
                                  Suggested Name: "Spoonity Enterprise Platform
                                  Fee" - Billed directly to corporate office
                                </div>
                              </div>
                              ...
                              <div className="col-span-2 bg-orange-50 rounded-lg p-4 border border-orange-200">
                                ...
                                <div className="grid grid-cols-2 gap-2 text-sm mb-3">
                                  ...
                                  <div className="col-span-2 mt-2">
                                    <div className="flex justify-between text-gray-700">
                                      ...
                                      <span className="font-medium">
                                        {formatCurrency(
                                          feeBreakdown.franchisee +
                                            (giftCard ? stores * 30 : 0)
                                        )}
                                      </span>
                                    </div>
                                ...
                                <div className="text-xs text-gray-600">
                                  Suggested Name: "Spoonity Location Connection
                                  Fee" - Billable to individual franchise
                                  locations
                                </div>
                              </div>
                            </>
                          ...
                          <div className="col-span-2 mt-2">
                            <div className="text-xs text-gray-600 pl-4 space-y-1">
                              ...
                              {appType === "premium" && (
                                <p>
                                  • Premium App Setup: {formatCurrency(15000)}
                                </p>
                              )}
                              {appType === "standard" && (
                                <p>
                                  • Standard App Setup: {formatCurrency(5000)}
                                </p>
                              )}
                              {appType === "pwa" && (
                                <p>• PWA Setup: {formatCurrency(1000)}</p>
                              )}
                              {dataIngestion && (
                                <p>
                          ...
                          <div className="text-right text-base font-bold spoonity-primary-text pt-2">
                            {formatCurrency(monthlyFees * 12 + setupFees)}
                          </div>
                        </div>
                      </div>
                      ...
                      <div>
                        <div className="bg-gray-50 rounded-lg p-4 mb-6">
                          ...
                          <p className="text-sm text-gray-600">
                            ...
                            provided. A Spoonity representative will contact you
                            to provide a final quote and answer any questions.
                            Pricing is subject to change based on specific
                      ...
                      <div className="mt-8">
                        ...
                        <p className="text-xs text-gray-500 mt-2">
                          By submitting, a Spoonity representative will contact
                          you with a custom quote based on your selections.
                        </p>
                      </div>
                    </div>
                  </div>
