...
export default function SpoonityCalculator() {
  ...
  const saveTokenAndData = (token: string) => {
    ...
    localStorage.setItem('spoonity_user_data', JSON.stringify({
      firstName,
      lastName,
      email,
      phone,
  ...
  React.useEffect(() => {
    ...
    if (storedToken && expiryTime && parseInt(expiryTime) > Date.now() && userData) {
      ...
      setCountry(parsedUserData.country === 'Other' ? 'Other' : parsedUserData.country);
      setOtherCountry(parsedUserData.country === 'Other' ? parsedUserData.country : '');
  ...
  const getQuoteDetails = () => {
    ...
    if (giftCard) {
      ...
      addOns.push(`Gift Card Per-Store Fee (${stores} stores @ $30/store): ${formatCurrency(stores * 30)}`);
    }
    ...
    if (whatsappEnabled) {
      ...
      addOns.push(`WhatsApp Per-Store Fee (${stores} stores): ${formatCurrency(feeBreakdown.whatsapp.perStore)}/month`);
      addOns.push(`WhatsApp Message Fees (${whatsappCountry}):`);
      addOns.push(`  • Digital Receipts: ${formatCurrency(whatsappMarketTicket * whatsappRates[whatsappCountry].marketTicket)}/month`);
      addOns.push(`  • Utility Messages: ${formatCurrency(whatsappUtility * whatsappRates[whatsappCountry].utility)}/month`);
      addOns.push(`  • Marketing Messages: ${formatCurrency(whatsappMarketing * whatsappRates[whatsappCountry].marketing)}/month`);
      addOns.push(`  • OTP Messages: ${formatCurrency(whatsappOtp * whatsappRates[whatsappCountry].otp)}/month`);
    }
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              <div>
                ...
                <input
                  id="token"
                  type="text"
                  className={`w-full border rounded-md p-2.5 input-field ${tokenError ? 'border-red-500' : ''}`}
                  value={token}
                  onChange={(e) => {
          ...
          <div className="spoonity-primary p-6 text-white">
            <div className="flex items-center justify-between">
              ...
              <div className="text-sm text-right hidden sm:block">
                ...
                <div className="font-medium text-white">{firstName} {lastName} • <button onClick={handleSignOut} className="text-white opacity-90 hover:underline">Sign Out</button></div>
              </div>
            </div>
          </div>
                    ...
                    <div className="ml-7 space-y-4">
                      ...
                      <div>
                        ...
                        <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
                          <div>
                            <label className="block text-xs text-gray-600 mb-1">
                              Digital Receipts (${whatsappRates[whatsappCountry].marketTicket.toFixed(4)}/msg)
                              <span className="text-gray-500 ml-1">- Digital receipts sent to customers</span>
                            </label>
                            <input
                              type="number"
                              min="0"
                          ...
                          <div>
                            <label className="block text-xs text-gray-600 mb-1">
                              Utility Campaign Messages (${whatsappRates[whatsappCountry].utility.toFixed(2)}/msg)
                              <span className="text-gray-500 ml-1">- Service notifications and updates</span>
                            </label>
                            <input
                              type="number"
                              min="0"
                          ...
                          <div>
                            <label className="block text-xs text-gray-600 mb-1">
                              Marketing Campaign Messages (${whatsappRates[whatsappCountry].marketing.toFixed(2)}/msg)
                              <span className="text-gray-500 ml-1">- Promotional messages and offers</span>
                            </label>
                            <input
                              type="number"
                              min="0"
                          ...
                          <div>
                            <label className="block text-xs text-gray-600 mb-1">
                              Authentication OTP Messages (${whatsappRates[whatsappCountry].otp.toFixed(2)}/msg)
                              <span className="text-gray-500 ml-1">- One-time passwords and verification codes</span>
                            </label>
                            <input
                              type="number"
                              min="0"
                        ...
                        <div className="mt-2 text-xs text-gray-500">
                          Default calculation: 10% of total monthly transactions ({Math.round(stores * transactions * 0.1).toLocaleString()} messages) for receipts, utility, and OTP messages
                        </div>
                      </div>
                      ...
                      <div className="pt-2 text-xs bg-gray-50 p-3 rounded-md">
                        ...
                        <div className="grid grid-cols-2 gap-1">
                          ...
                          <span className="font-medium pt-2">Message Fees Breakdown:</span>
